# Rscript ~/thirdparty/AnnotateSNPs/annotate_snps_trans.R /sc/orga/scratch/cohaia01/MGH/SnpsInterestedIn /sc/orga/scratch/cohaia01/MGH/Expression/MGH/subq_PCA.txt /sc/orga/scratch/cohaia01/MGH/subq_trans_annotation.xls
# Rscript ~/thirdparty/AnnotateSNPs/annotate_snps_trans.R /sc/orga/scratch/cohaia01/MGH/SnpsInterestedIn /sc/orga/scratch/cohaia01/MGH/Expression/liver_PCA.txt /sc/orga/scratch/cohaia01/MGH/liver_trans_annotation.xls
# Rscript ~/thirdparty/AnnotateSNPs/annotate_snps_trans.R /sc/orga/scratch/cohaia01/MGH/SnpsInterestedIn /sc/orga/scratch/cohaia01/MGH/Expression/MGH/omental_PCA.txt /sc/orga/scratch/cohaia01/MGH/omental_trans_annotation.xls
...
#args <- c("/sc/orga/scratch/cohaia01/MGH/SnpsInterestedIn","/sc/orga/scratch/cohaia01/MGH/Expression/MGH/subq_PCA.txt", "/sc/orga/scratch/cohaia01/MGH/subq_trans_annotation.xls")
...
output = foreach(chr  = names(split_snps_by_chr),combine= rbind)%dopar%{
  ...
  # run_eqtlResults(chr, geneAnnotationFN,tmp,tmp2, tmp3 )
    ...
    for(snp in snpsInterest$marker){
      ...
      if(length(trans_snp_tmp)>0){
        trans_snp=rbind(trans_snp,trans_snp_tmp)
        names = c(names, snp)  
      }
      
    }
    rownames(trans_snp) = names
    ...
    if(nrow(eqtls)>0){
    res = getMeansStdErrs(eqtls,geneME,snpsME)
    }else{
      res = NA
    }
    cat("---finished: ",chr,"---\n")
